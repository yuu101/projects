...
class Book_model extends CI_Model {
	...
	function __construct()		//constructer    繼承CI_Modle的constructer
    {
        parent::__construct();
            ...
            // SELECT authro.`name` AS authorName, publicher.`name` AS publisherName , book.`name` AS bookName
            // FROM book
            ...
            //      JOIN `author` ON author.`id` = book.`author_id`
            //      JOIN `publisher` ON publisher.`id` = book.`publisher_id`
            //      JOIN `copy_book` ON copy.`isbn` = book.`isbn`
            //      LEFT JOIN `borrow_return` ON borrow_return.`c_id` = copy_book.`c_id
            //      WHERE return_date != null
            //      ) AS thisCategory
            // WHERE `categoty` = '文學'
            //      
            //       
            // SELECT category.`name` COUNT(isbn)
            // FROM book
